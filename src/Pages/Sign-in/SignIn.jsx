    ...
    <div className="min-h-full w-full flex justify-center items-center mb-20">
      <div className="flex flex-col w-[95%] h-fit md:flex-row mt-10 bg-black md:h-[650px] md:w-[1200px] rounded-3xl gap-20 md:gap-0 md:items-center md:justify-between outline-4 md:outline-4 outline-amber-300">
        ...
        <div className="md:w-1/2 md:h-full md:mt-52 rounded-r-3xl mb-8">
          <form action="" className="ml-5 flex flex-col" onSubmit={singIn}>
            <button className="h-14 w-[95%] md:w-[80%] bg-white mb-10 rounded-full flex flex-row items-center gap-1 justify-center cursor-pointer hover:bg-white/90 transition-opacity transform-fill" >
                        <img src={google} alt="" className="h-8 w-10"/><p className=" text-xl lg:text-2xl google-font">Sign in with Google </p>
                        </button>
            <div className="flex flex-col  gap-1">
              <label htmlFor="Email" className="text-2xl md:text-3xl text-white">
                Email
              </label>
              <input className=" h-12 md:h-14 w-[95%] md:w-[80%] bg-[#D9D9D9] rounded-lg px-2 text-xl" name="mail"></input>
            </div>
